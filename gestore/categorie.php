...
<head>
    ...
    <title>Gestione Categorie | Pizzeria da Paggi</title>
...
<body>
    <div class="w3-container bg-pizza w3-padding-16">
        <h2 class="w3-xlarge w3-margin-0">
            <i class="fa fa-list"></i> Gestione Categorie Pizzeria da Paggi
        </h2>
    </div>
    ...
    <div class="w3-bar w3-white w3-card w3-margin-bottom">
        ...
        <div class="w3-right">
            <a href="gestione_categorie.php" class="w3-bar-item w3-button bg-basil w3-padding-16">
                <i class="fa fa-plus"></i> Inserisci Categoria
            </a>
        </div>
    </div>
    ...
    <div class="w3-content" style="max-width:1200px">
        ...
        <div class="w3-container w3-padding-16 bg-cream w3-card w3-round-large w3-margin">
            <form method="GET">
                <h4 class="text-pizza w3-border-bottom w3-padding-16"><i class="fa fa-search"></i> Filtra Categorie</h4>
                <div class="w3-row-padding">
                    <div class="w3-half">
                        <label>Nome Categoria</label>
                        <input class="w3-input w3-border w3-round" type="text" name="nome" placeholder="Cerca categoria..." value="<?= $_GET['nome'] ?? '' ?>">
                    </div>
                    <div class="w3-half">
                        <label>Visibilità</label>
                        <select class="w3-select w3-border w3-round" name="visibile">
                            <option value="">Tutte</option>
                            <option value="1" <?= ($_GET['visibile'] ?? '') === '1' ? 'selected' : ''; ?>>Visibili</option>
                            <option value="0" <?= ($_GET['visibile'] ?? '') === '0' ? 'selected' : ''; ?>>Nascoste</option>
                        </select>
                    </div>
                </div>
        ...
        <div class="w3-container">
            <div class="w3-panel w3-white w3-card w3-border-left w3-border-red">
                <p>
                    <?php if ($num_record > 0): ?>
                        <i class="fa fa-info-circle text-pizza"></i> Nell'archivio sono presenti <strong><?= $num_record ?></strong> categorie.
                    <?php else: ?>
                        <i class="fa fa-exclamation-triangle w3-text-red"></i> Non ci sono categorie memorizzate in archivio.
                    <?php endif ?>
                </p>
            </div>
        </div>
                        ...
                        <tbody>
                            <?php foreach ($categorie as $cat): ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php if ($cat['visibile']): ?>
                                            <span class="w3-text-green w3-bold">VISIBILE</span>
                                        <?php else: ?>
                                            <span class="w3-text-red w3-bold">NASCOSTO</span>
                                        <?php endif; ?>
                                    </td>
                                    <td class="w3-center">
                                        <a href="gestione_categorie.php?id=<?= $cat["id_categoria"] ?>" title="Modifica">
                                            <i class="fa fa-edit w3-text-blue"></i>
                                        </a>
                                    </td>
                                    <td class="w3-center">
                                        <a href="elimina.php?id=<?= $cat["id_categoria"] ?>&tabella=categorie" title="Elimina" onclick="return confirm('Vuoi eliminare questa categoria?')">
